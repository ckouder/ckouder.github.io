...
<script lang="ts">
	...
	import { FULL_WINDOW_MARGIN, MIN_SIZE, TRANSITION } from '../constant';
	...
	export function resize(
		...
		doWithAnimation(withAnimation, () => {
			...
			w.style.width = window.innerWidth - size.left - size.right + 'px';
			w.style.height = window.innerHeight - size.top - size.bottom + 'px';
		});
		windowSize = { ...size };
	...
	export function reposition(x: number, y: number, triggerEvent: boolean = true) {
	...
	function resizeHandler(e: CustomEvent) {
	...
	function onReposition(e: CustomEvent) {
		if (disabled) return;
		reposition(e.detail.x + repositionOffset.left, e.detail.y + repositionOffset.top);
	}
...
>
	...
	<div class="resize-controller" hidden={resizeDisabled}>
		<div class="ctrl ctrl-left" data-tag="left" use:mouseTracker on:trackStart={resizeHandler} />
		<div class="ctrl ctrl-top" data-tag="top" use:mouseTracker on:trackStart={resizeHandler} />
		<div class="ctrl ctrl-right" data-tag="right" use:mouseTracker on:trackStart={resizeHandler} />
		<div
